<template>
  <div>
    <div class="card" style="margin-bottom: 5px;">
      <el-input v-model="data.userName" placeholder="请输入学生名称查询" style="width: 240px"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="candidateNumber" label="考生号"></el-table-column>
        <el-table-column prop="examinationNumber" label="准考证号"></el-table-column>
        ...
        <el-table-column label="身份证照片">
          <template #default="scope">
            <el-image style="width: 40px; height: 40px; border-radius: 5px" v-if="scope.row.identityImg" :src="scope.row.identityImg" :preview-src-list="[scope.row.identityImg]" preview-teleported></el-image>
          </template>
        </el-table-column>
    ...
    <el-dialog title="学籍认证信息" v-model="data.formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
...
// 加载表格数据
const load = () => {
  request.get('/userCertification/selectPage', {
    params: {
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/userCertification/delete/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/userCertification/delete/batch', {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()  // 刷新表格数据
      } else {
