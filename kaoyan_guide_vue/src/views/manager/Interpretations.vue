<template>
  <div>
    <div class="card" style="margin-bottom: 5px;">
      <el-input v-model="data.name" placeholder="请输入解读标题查询" style="width: 240px"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" tooltip-effect="dark myTooltip" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="解读标题" show-overflow-tooltip></el-table-column>
        <el-table-column prop="categorysName" label="门类名称"></el-table-column>
        <el-table-column prop="intro" label="解读简介" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog title="专业解读" v-model="data.formVisible" width="50%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" :rules="data.rules" label-width="80px"  style="padding: 20px 30px" ref="formRef">
        <el-form-item label="解读标题" prop="name">
          <el-input v-model="data.form.name" placeholder="解读标题"></el-input>
        </el-form-item>
        <el-form-item label="所属门类" prop="categorysId">
          <el-select style="width: 100%" v-model="data.form.categorysId" placeholder="请选择所属门类">
            <el-option v-for="item in data.categorysList" :key="item.id" :value="item.id" :label="item.name"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="解读简介" prop="intro">
          <el-input v-model="data.form.intro" placeholder="解读简介" type="textarea" rows="4"></el-input>
        </el-form-item>
        ...
        <el-form-item label="解读详情" prop="content">
...
const data = reactive({
  ...
  rules: {
    name: [
      { required: true, message: '请输入解读标题', trigger: 'blur' },
    ],
    categorysId: [
      { required: true, message: '请选择所属门类', trigger: 'blur' },
    ],
    intro: [
      { required: true, message: '请输入解读简介', trigger: 'blur' },
    ],
    img: [
      { required: true, message: '请上传封面图片', trigger: 'blur' },
    ],
    content: [
      { required: true, message: '请输入解读详情', trigger: 'blur' },
    ],
  },
  categorysList: [],
  content: '',
...
// 新增
const add = () => {
  request.post('/interpretations/add', data.form).then(res => {
    if (res.code === '200') {
...
// 更新
const update = () => {
  request.put('/interpretations/update', data.form).then(res => {
    if (res.code === '200') {
      data.formVisible = false
      ElMessage.success('操作成功')
      load()
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/interpretations/delete/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/interpretations/delete/batch', {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()  // 刷新表格数据
      } else {
