...
const data = reactive({
  ...
  name: null,
  rules: {
    name: [{ required: true, message: "请输入政策标题", trigger: "blur" }],
    intro: [{ required: true, message: "请输入政策简介", trigger: "blur" }],
    content: [{ required: true, message: "请输入政策详情", trigger: "blur" }],
  },
  universityList: [],
  content: "",
...
const delBatch = () => {
  ...
  ElMessageBox.confirm("删除后数据无法恢复，您确定删除吗?", "删除确认", {
    ...
    .then((res) => {
      request
        .delete("/policys/delete/batch", { data: data.ids })
        .then((res) => {
          if (res.code === "200") {
            ElMessage.success("操作成功");
            load(); // 刷新表格数据
