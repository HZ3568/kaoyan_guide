<template>
  <div>
    <div class="card" style="margin-bottom: 5px;">
      <el-input v-model="data.name" placeholder="请输入政策标题查询" style="width: 240px"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" tooltip-effect="dark myTooltip" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="intro" label="政策简介" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog title="招生政策" v-model="data.formVisible" width="50%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" :rules="data.rules" label-width="80px"  style="padding: 20px 30px" ref="formRef">
        <el-form-item label="政策标题" prop="name">
          <el-input v-model="data.form.name" placeholder="政策标题"></el-input>
        </el-form-item>
        <el-form-item label="政策简介" prop="intro">
          <el-input v-model="data.form.intro" placeholder="政策简介" type="textarea" rows="4"></el-input>
        </el-form-item>
        <el-form-item label="政策详情" prop="content">
...
const data = reactive({
  ...
  rules: {
    ...
    content: [
      { required: true, message: '请输入政策详情', trigger: 'blur' },
    ],
  },
  universityList: [],
  content: '',
...
// 加载表格数据
const load = () => {
  request.get('/policys/selectPage', {
    params: {
...
// 新增
const add = () => {
  request.post('/policys/add', data.form).then(res => {
    if (res.code === '200') {
...
// 更新
const update = () => {
  request.put('/policys/update', data.form).then(res => {
    if (res.code === '200') {
      data.formVisible = false
      ElMessage.success('操作成功')
      load()
...
const del = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/policys/delete/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/policys/delete/batch', {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()  // 刷新表格数据
      } else {
